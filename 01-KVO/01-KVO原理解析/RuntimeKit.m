...
/**
 2、根据类获取类名
 class_rw_t结构体中有 char *demangledName; 这就是类名
 */
...
/**
 3、获取成员变量
 class_ro_t 结构体中有 ivar_list_t，遍历获取Ivar
...
+ (NSArray *)fetchPropertyList:(Class)cls {
    ...
    for (unsigned int i = 0; i < count; i ++) {
        ...
        NSDictionary *propertyDic = @{
            @"propertyName":[NSString stringWithUTF8String:propertyName],
...
+ (NSArray *)fetchMethodList:(Class)cls {
    ...
    for (unsigned int i = 0; i < count; i ++) {
        ...
        NSDictionary *methodDic = @{
            @"methodName":NSStringFromSelector(methodName),
...
+ (NSArray *)fetchProtocolList:(Class)cls {
    unsigned int count = 0;
    __unsafe_unretained Protocol **potocolList = class_copyProtocolList(cls, &count);
    NSMutableArray *mutableArray = [NSMutableArray arrayWithCapacity:count];
    for (unsigned int i = 0; i < count; i ++) {
