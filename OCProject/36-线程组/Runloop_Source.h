...
// eocclass
// mode 结构，每一个mode都有自己的事情处理 defalutModel， trackingModel， InitModel
// runloop一次只能执行一个model
...
#endif
#if USE_MK_TIMER_TOO
    mach_port_t _timerPort;// dispatchPort(GCD 定时器)
...
#if USE_MK_TIMER_TOO
    /*
     NStimer的触发时间是根据mk_arm来配置（软硬截止时间）
     NSTimer的方法myTimer掉用，是通过timerport来通知runloop，需要处理定时器了，runloop来处理
     */
    rlm->_timerPort = mk_timer_create(); // eocclass  mk_time来创建的
...
/*
 线程与runloop关系
 就是 一个静态的字典__CFRunLoops保存起来（key就是线程，value就是runloop）
 */
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    ...
    if (!__CFRunLoops) { // 创建__CFRunLoops 字典
        // 第一个runloop（currentLoop）（mainrunloop）
    ...
    // 根据线程 获取runloop  key 就是线程， value 就是我们的 CFRunLoopRef
...
CFRunLoopRef CFRunLoopGetCurrent(void) {
    ...
    if (rl) return rl;
    return _CFRunLoopGet0(pthread_self()); //CFRunLoopGetCurrent 核心方法 _CFRunLoopGet0
}
...
static Boolean __CFRunLoopDoSources0(CFRunLoopRef rl, CFRunLoopModeRef rlm, Boolean stopAfterHandle) {	/* DOES CALLOUT */
    ...
    if (NULL != sources) {
        ...
        __CFRunLoopModeUnlock(rlm);
        __CFRunLoopUnlock(rl);
        // sources is either a single (retained) CFRunLoopSourceRef or an array of (retained) CFRunLoopSourceRe
    ...
    if (rlm->_timers) {
        ...
        if (nextSoftDeadline < UINT64_MAX && (nextHardDeadline != rlm->_timerHardDeadline || nextSoftDeadline != rlm->_timerSoftDeadline)) {
            ...
            if (leeway > 0) {
                ...
                // Arm the mk timer
                if (rlm->_timerPort) { // 如果有这个timerPort 1 (dispatchPort source1)
                    // mk_timer_arm 配置下一次的触发事件 内核  （eocclass MK）
...
static Boolean __CFRunLoopDoTimer(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFRunLoopTimerRef rlt) {	/* DOES CALLOUT */
    ...
    if (__CFIsValid(rlt) && rlt->_fireTSR <= mach_absolute_time() && !__CFRunLoopTimerIsFiring(rlt) && rlt->_runLoop == rl) {
        ...
        __CFRunLoopModeUnlock(rlm);
        __CFRunLoopUnlock(rl);
        // 执行定时器（执行定时器的方法） 执行myTimer方法 eocclass
...
static Boolean __CFRunLoopDoTimers(CFRunLoopRef rl, CFRunLoopModeRef rlm, uint64_t limitTSR) {	/* DOES CALLOUT */
    ...
    for (CFIndex idx = 0, cnt = rlm->_timers ? CFArrayGetCount(rlm->_timers) : 0; idx < cnt; idx++) {
        ...
        if (__CFIsValid(rlt) && !__CFRunLoopTimerIsFiring(rlt)) {
            if (rlt->_fireTSR <= limitTSR) { // 判断时间
                if (!timers) timers = CFArrayCreateMutable(kCFAllocatorSystemDefault, 0, &kCFTypeArrayCallBacks);
                CFArrayAppendValue(timers, rlt); // 需要处理的定时器放到array
            }
        }
    }
    ...
    for (CFIndex idx = 0, cnt = timers ? CFArrayGetCount(timers) : 0; idx < cnt; idx++) {
        ...
        Boolean did = __CFRunLoopDoTimer(rl, rlm, rlt);// 单个处理定时器
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    ...
    // GCD 定时器 （dispatchPort 通讯） （runloop有关系）
    ...
    do {
        ...
        if (MACH_PORT_NULL != dispatchPort && !didDispatchPortLastTime) {
            ...
            if (__CFRunLoopServiceMachPort(dispatchPort, &msg, sizeof(msg_buffer), &livePort, 0, &voucherState, NULL)) {
                //
                goto handle_msg// (handle_msg sourece1,)
            }
#elif DEPLOYMENT_TARGET_WINDOWS
  ...
  if (kCFUseCollectableAllocator) {
        ...
        // 通知监听者，我醒来了，要处理事情，但是我要处理什么样的事情呢
    ...
    handle_msg:
        ...
        if (MACH_PORT_NULL == livePort) {
            ...
            // 处理定时器 每个一秒触发（NStimer） 1秒的时候触发了， 第二秒要触发（）
...
#endif
        // NStimer  在commond模式下，gcd
        // GCD （runloop） gcd有事情要处理了，通过dispatchPort来通知runloop，我有事情要处理了
        // runloop掉用__CFRUNLOOP_IS_SERVICING_THE_MAIN_DISPATCH_QUEUE__
        // __CFRUNLOOP_IS_SERVICING_THE_MAIN_DISPATCH_QUEUE__ 又回到了GCD的库
        else if (livePort == dispatchPort) {// GCD 消息处理
